...
{
    public partial class frmItemsOutOnBail : Edex.GeneralObjects.GeneralForms.BaseForm
    {
        #region Declare
        DataTable dtDeclaration;
        public const int DocumentType = 12;
        int rowIndex;
        string Barcode = "";
        string columnName;
        ...
        private Stc_ItemsOutonBailDAL cClass;
        ...
        BindingList<Stc_ItemsOutonBail_Details> AllRecords = new BindingList<Stc_ItemsOutonBail_Details>();

        //list detail
        BindingList<Stc_ItemsOutonBail_Details> lstDetail = new BindingList<Stc_ItemsOutonBail_Details>();

        //Detail
        Stc_ItemsOutonBail_Details BoDetail = new Stc_ItemsOutonBail_Details();

        #endregion
        public frmItemsOutOnBail()
        {
            try
            {
                ...
                /***********************Component ReadOnly  ****************************/
                TextEdit[] txtEdit = new TextEdit[5];
                txtEdit[0] = lblStoreName;
                txtEdit[1] = lblStoreName;
                txtEdit[2] = lblCostCenterName;
                txtEdit[3] = lblDebitAccountName;
                txtEdit[4] = lblCreditAccountName;
                ...
                cmbCurency.EditValue = MySession.GlobalDefaultCurencyID;

                txtInvoiceDate.ReadOnly = MySession.GlobalCanChangeDocumentsDate;
                ...
                /************TextEdit Account ID ***************/
                lblDebitAccountID.ReadOnly = MySession.GlobalAllowChangefrmGoodsOpeningDebitAccountID;
        ...
        void InitGrid()
        {
            lstDetail = new BindingList<Stc_ItemsOutonBail_Details>();
            ...
            gridView1.Columns["StoreID"].Visible = true;
            ...
            gridView1.Columns["Caliber"].Caption = "العيار";
        ...
        private void gridView1_ValidateRow(object sender, DevExpress.XtraGrid.Views.Base.ValidateRowEventArgs e)
        {
            try
            {
                ...
                {
                    if (col.FieldName == "PackingQty" || col.FieldName == "SizeID" || col.FieldName == "CostPrice")
                    {
        ...
        private void gridView1_ValidatingEditor(object sender, DevExpress.XtraEditors.Controls.BaseContainerValidateEditorEventArgs e)
        {
            ...
            else if (this.gridView1.ActiveEditor is TextEdit)
            {
                ...
                if (ColName == "SizeID" || ColName == "PackingQty" || ColName == "GroupID" || ColName == "CostPrice")
                {
                    if (val == null || string.IsNullOrWhiteSpace(val.ToString()))
                    {
                        e.Valid = false;
                    ...
                    else if (ColName == "SizeID")
                    {
                        ...
                        if (dt.Rows.Count == 0)
                        {
                            DataTable dtSize = Lip.SelectRecord("SELECT SizeID, " + PrimaryName + " AS " + SizeName + " FROM Stc_SizingUnits Where Cancel=0 And SizeID=" + Comon.cInt(val.ToString()) + " And FacilityID=" + UserInfo.FacilityID);
                    ...
                    {
                        DataTable dtItemGroup = Lip.SelectRecord("SELECT GroupID, " + PrimaryName + " AS " + GroupName + " FROM Stc_ItemsGroups Where Cancel=0 And GroupID=" + Comon.cInt(val.ToString()) + " And FacilityID=" + UserInfo.FacilityID);
                ...
                {
                    DataTable dtItemGroup = Lip.SelectRecord("Select GroupID, " + PrimaryName + " AS " + GroupName + " from Stc_ItemsGroups Where Cancel=0 And LOWER (" + PrimaryName + ")=LOWER ('" + val.ToString() + "') And FacilityID=" + UserInfo.FacilityID);
                    if (dtItemGroup.Rows.Count == 0)
                    {
                        e.Valid = false;
                        HasColumnErrors = true;
            ...
            try
            {
                ...
                else if (e.KeyCode == Keys.Tab || e.KeyCode == Keys.Enter)
                {
                    if (view.ActiveEditor is TextEdit)
                    {
                        ...
                        string ColName = view.FocusedColumn.FieldName;
                        if (ColName == "PackingQty" || ColName == "SizeID" || ColName == "CostPrice")
                        {
        ...
        private void FileItemData(DataTable dt)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                gridView1.SetRowCellValue(gridView1.FocusedRowHandle, gridView1.Columns["QTY"], dt.Rows[0]["QTY"].ToString());
        ...
        bool IsValidGrid()
        {
            ...
            for (int i = 0; i < length; i++)
            {
                foreach (GridColumn col in gridView1.Columns)
                {
                    if (col.FieldName == "SizeID" /*|| col.FieldName == "PackingQty"*/  || col.FieldName == "CostPrice")
                    {
        ...
        public void SumTotalBalanceAndDiscount(int row = -1)
        {
            try
            {
                ...
                for (int i = 0; i <= gridView1.DataRowCount - 1; i++)
                {
                    ...
                    DiamondRow +=Comon.ConvertToDecimalQty(gridView1.GetRowCellValue(i, "DIAMOND_W").ToString());
                    ...
                    //سعر تكلفة مع مصاريف
                    decimal SpendPrice = Comon.ConvertToDecimalPrice(CostPriceRow );
                    //سعر تكلفة المحل
                    ...
                    gridView1.SetRowCellValue(i, gridView1.Columns["Equivalen"], QTYRowEquivalen.ToString());
                ...
                if (rowIndex < 0)
                {
                    ...
                    DiamondRow = gridView1.GetRowCellValue(rowIndex, "DIAMOND_W") != null ? Comon.cDec(gridView1.GetRowCellValue(rowIndex, "DIAMOND_W")) : 0;
                    ...
                    int Caliber = Comon.cInt(gridView1.GetRowCellValue(rowIndex, "Caliber"));
                    QTYRowEquivalen = Comon.ConvertTo21Caliber(QTYRow, Caliber);
                    ...
                    gridView1.SetRowCellValue(rowIndex, gridView1.Columns["Equivalen"], QTYRowEquivalen.ToString());
        ...
        public bool Find()
        {
            ...
            {
                if (!MySession.GlobalAllowChangefrmPurchaseReturnStoreID) { Messages.MsgExclamationk(Messages.TitleInfo, Messages.msgNoPermissionToChange); return false; };
            ...
            {
                if (!MySession.GlobalAllowChangefrmPurchaseReturnCostCenterID) { Messages.MsgExclamationk(Messages.TitleInfo, Messages.msgNoPermissionToChange); return false; };
            ...
            if (cls.PrimaryKeyValue != null && cls.PrimaryKeyValue.ToString() != "")
            {
                ...
                else if (FocusedControl == gridControl.Name)
                {
                    ...
                    else if (gridView1.FocusedColumn.Name == "colSizeName" || gridView1.FocusedColumn.Name == "colSizeID")
                    {
                        ...
                        AddRow();
                        int SizeID = Comon.cInt(cls.PrimaryKeyValue.ToString());
                        DataTable dtSize = Lip.SelectRecord("SELECT SizeID, " + PrimaryName + " AS " + SizeName + " FROM Stc_SizingUnits Where Cancel=0 And SizeID=" + SizeID);
                        gridView1.SetRowCellValue(gridView1.FocusedRowHandle, gridView1.Columns["SizeID"], dtSize.Rows[0]["SizeID"].ToString());
                        gridView1.SetRowCellValue(gridView1.FocusedRowHandle, gridView1.Columns[SizeName], dtSize.Rows[0][SizeName].ToString());
                        gridView1.SetRowCellValue(gridView1.FocusedRowHandle, gridView1.Columns["StoreID"], txtStoreID.Text);
                        gridView1.SetRowCellValue(gridView1.FocusedRowHandle, gridView1.Columns["FacilityID"], UserInfo.FacilityID);
                    ...
                    else if (gridView1.FocusedColumn.Name == ("col" + GroupName) || gridView1.FocusedColumn.Name == "colGroupID")
                    {
                        ...
                        DataTable dtItemGroup = Lip.SelectRecord("SELECT GroupID, " + PrimaryName + " AS " + GroupName + " FROM Stc_ItemsGroups Where Cancel=0 And GroupID=" + GroupID);
                        gridView1.SetRowCellValue(gridView1.FocusedRowHandle, gridView1.Columns["GroupID"], dtItemGroup.Rows[0]["GroupID"].ToString());
                        gridView1.SetRowCellValue(gridView1.FocusedRowHandle, gridView1.Columns[GroupName], dtItemGroup.Rows[0][GroupName].ToString());
                        gridView1.SetRowCellValue(gridView1.FocusedRowHandle, gridView1.Columns["StoreID"], txtStoreID.Text);
                        gridView1.SetRowCellValue(gridView1.FocusedRowHandle, gridView1.Columns["FacilityID"], UserInfo.FacilityID);
        ...
        public void ReadRecord(long InvoiceID, bool flag = false)
        {
            try
            {
                ...
                ClearFields();
                {
                    ...
                    dt = Stc_ItemsOutonBailDAL.frmGetDataDetailByID(InvoiceID, Comon.cInt(cmbBranchesID.EditValue), UserInfo.FacilityID);
                    ...
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        ...
                        //Account
                        txtDebitGoldAccountID.Text = dt.Rows[0]["DebitGoldAccount"].ToString();
                        txtDebitGoldAccountID_Validating(null, null);


                        lblDebitAccountID.Text = dt.Rows[0]["DebitAccount"].ToString();
                        lblDebitAccountID_Validating(null, null);

                        txtSupplierID.Text = dt.Rows[0]["DebitAccount"].ToString();
                        txtSupplierID_Validating(null, null);
        ...
        public void ClearFields()
        {
            try
            {
                ...
                txtCostCenterID_Validating(null, null);
                txtStoreID.Text = MySession.GlobalDefaultStoreID.ToString();
                txtStoreID_Validating(null, null);
        ...
        public void MoveRec(long PremaryKeyValue, int Direction)
        {
            try
            {
                ...
                if (FormView == true)
                {
                    strSQL = "SELECT TOP 1 * FROM " + Stc_ItemsOutonBailDAL.TableName + " Where Cancel =0 ";
                    switch (Direction)
                    {
                        case xMoveFirst:
                            {
                                strSQL = strSQL + " ORDER BY " + Stc_ItemsOutonBailDAL.PremaryKey + " ASC";
                                break;
                            }

                        case xMoveNext:
                            {
                                strSQL = strSQL + " And " + Stc_ItemsOutonBailDAL.PremaryKey + ">" + PremaryKeyValue + " ORDER BY " + Stc_ItemsOutonBailDAL.PremaryKey + " asc";
                                break;
                            }

                        case xMovePrev:
                            {
                                strSQL = strSQL + " And " + Stc_ItemsOutonBailDAL.PremaryKey + "<" + PremaryKeyValue + " ORDER BY " + Stc_ItemsOutonBailDAL.PremaryKey + " desc";
                                break;
                            }

                        case xMoveLast:
                            {
                                strSQL = strSQL + " ORDER BY " + Stc_ItemsOutonBailDAL.PremaryKey + " DESC";
                                break;
                            }
                    }
                    cClass = new Stc_ItemsOutonBailDAL();
        ...
        protected override void DoNew()
        {
            ...
            {
                IsNewRecord = true;
                txtInvoiceID.Text = Stc_ItemsOutonBailDAL.GetNewID(UserInfo.FacilityID, UserInfo.BRANCHID, UserInfo.ID).ToString();
                ClearFields();
                EnabledControl(true);
                txtStoreID.Focus();
        ...
        protected override void DoEdit()
        {
            try
            {
                ...
                for (int i = 0; i <= gridView1.DataRowCount - 1; i++)
                {
                    ...
                    dtItem.Rows[i]["DIAMOND_W"] = Comon.ConvertToDecimalQty(gridView1.GetRowCellValue(i, "DIAMOND_W").ToString());
                    dtItem.Rows[i]["BAGET_W"] = Comon.ConvertToDecimalQty(gridView1.GetRowCellValue(i, "BAGET_W").ToString());
                    dtItem.Rows[i]["STONE_W"] = Comon.ConvertToDecimalQty(gridView1.GetRowCellValue(i, "STONE_W").ToString());
            ...
            {
                Messages.MsgError("Error:", ex.Message);
            }
        }
        private void AddItems(int Rowindex)
        {
            try
            {
                ...
                Lip.AddNumericField("GroupID", gridView1.GetRowCellValue(Rowindex, "GroupID").ToString());
                ...
                long itemID = Comon.cLong(gridView1.GetRowCellValue(Rowindex, "ItemID").ToString());
                ...
                Lip.AddNumericField("ItemID", itemID.ToString());
                BarCode = (itemID * 10000).ToString() + SizeID.ToString();
                ...
                decimal SalePrice = CostPrice / 100 * 15;

                SalePrice = (CostPrice + SalePrice) * Comon.cDec(1.74);
                SalePrice = Comon.ConvertToDecimalPrice(SalePrice * 3);
                ...
                var strSQL1 = "Select * From Stc_ItemUnits where ItemID= " + itemID;
        ...
        protected override void DoSave()
        {
            try
            {
                ...
                for (int i = 0; i <= gridView1.DataRowCount - 1; i++)
                {
                    ...
                    if (gridView1.GetRowCellValue(i, "DIAMOND_W").ToString() != "")
                    {
                        ...
                        for (int j = 0; j < dtt.Rows.Count; j++)
                        {
                            ...
                            returned.CaptionOpration = "فاتورة صرف مخزني ";
                ...
                if (!Validations.IsValidForm(this))// valid form 
                    return;
                if (!IsValidGrid())// Check The Grid View  if any feild is error and is a requird
                    return;
        ...
        private void Save()
        {
            ...
            objRecord.StoreID = Comon.cInt(txtStoreID.Text);
            objRecord.OperationTypeName = (UserInfo.Language == iLanguage.English ? "GoodOpening Invoice" : "صرف مخزني ");
            txtNotes.Text = (txtNotes.Text.Trim() != "" ? txtNotes.Text.Trim() : (UserInfo.Language == iLanguage.English ? "GoodOpening Invoice" : " صرف مخزني "));
            objRecord.Notes = txtNotes.Text.Trim();
            objRecord.DocumentID = Comon.cInt(txtDocumentID.Text);

            //Account
            objRecord.DebitGoldAccount = Comon.cDbl(txtDebitGoldAccountID.Text);
            objRecord.DebitAccount = Comon.cDbl(lblDebitAccountID.Text);
            objRecord.CreditAccount = Comon.cDbl(lblCreditAccountID.Text);
            objRecord.InvoiceTotal = Comon.cDec(lblInvoiceTotal.Text);
            ...
            for (int i = 0; i <= gridView1.DataRowCount - 1; i++)
            {
                ...
                returned.ArbSizeName = Common.getWordArb(gridView1.GetRowCellValue(i, SizeName).ToString());
                ...
                returned.Equivalen = Comon.ConvertTo21Caliber(returned.QTY,Comon.cInt(returned.ArbSizeName));
                ...
                if (gridView1.GetRowCellValue(i, "StoreID") == null)
                    ...
                    returned.StoreID = Comon.cInt(gridView1.GetRowCellValue(i, "StoreID").ToString());

                returned.ExpiryDateStr = 01011900;
                ...
                returned.Cancel = 0;
                returned.Serials = "";
                if (returned.CostPrice <= 0 || returned.SizeID <= 0)
                    continue;
                listreturned.Add(returned);
            }
            ...
            if (listreturned.Count > 0)
            {
                ...
                objRecord.ItemsOutOnBailDatails = listreturned;
                int Result = Stc_ItemsOutonBailDAL.InsertUsingXML(objRecord, IsNewRecord);
                ...
                if (Comon.cInt(Result) > 0)
                {
                    ...
                    if (VoucherID == 0)
                        ...
                        Lip.ExecututeSQL("Update " + Stc_ItemsOutonBailDAL.TableName + " Set RegistrationNo =" + VoucherID + " where " + Stc_ItemsOutonBailDAL.PremaryKey + " = " + txtInvoiceID.Text);
        ...
        protected override void DoDelete()
        {
            try
            {
                ...
                for (int i = 0; i <= gridView1.DataRowCount - 1; i++)
                {
                    ...
                    FlageChecKDelete = Lip.CheckTheItemIsHaveTransactionByBarCode(gridView1.GetRowCellValue(i, "BarCode").ToString(), "Stc_ItemsInonBail_Details");
                    if (FlageChecKDelete)
                    {
                ...
                Stc_ItemsOutonBail_Master model = new Stc_ItemsOutonBail_Master();
                ...
                int Result = Stc_ItemsOutonBailDAL.Delete(model);
                if (Comon.cInt(Result) > 0)
                {
                    //حذف القيد الالي
        ...
        private void txtStoreID_Validating(object sender, CancelEventArgs e)
        {
            try
            {
                strSQL = "SELECT ArbName as StoreName FROM Stc_Stores WHERE StoreID =" + Comon.cInt(txtStoreID.Text) + " And Cancel =0 And  BranchID =" + Comon.cInt(cmbBranchesID.EditValue);
       ...
       /// <summary>
        /// This method checks the user's permissions and enables/disables search buttons accordingly. 
       /// </summary>
        private void RolesButtonSearchAccountID()
        {
            // Disable search button if user does not have permission to change debit account ID
            btnDebitSearch.Enabled = !MySession.GlobalAllowChangefrmGoodsOpeningDebitAccountID;
           
            // Disable search button if user does not have permission to change credit account ID
        ...
        private void btnDebitGoldSearch_Click(object sender, EventArgs e)
        {
            try
            {
                PrepareSearchQuery.SearchForAccounts(txtDebitGoldAccountID, lblDebitGoldAccountName);
            }
            catch (Exception ex)
            {
                Messages.MsgError(Messages.TitleError, this.GetType().Name + " " + System.Reflection.MethodBase.GetCurrentMethod().Name + " " + ex.Message);
        ...
        private void txtDebitGoldAccountID_Validating(object sender, CancelEventArgs e)
        {
            try
            {
                strSQL = "SELECT ArbName AS AccountName FROM Acc_Accounts WHERE (BranchID = " + Comon.cInt(cmbBranchesID.EditValue) + " ) AND " + " (Cancel = 0) AND (AccountID = " + txtDebitGoldAccountID.Text + ") ";
                CSearch.ControlValidating(txtDebitGoldAccountID, lblDebitGoldAccountName, strSQL);
            }
            catch (Exception ex)
            {
                Messages.MsgError(this.GetType().Name, System.Reflection.MethodBase.GetCurrentMethod().Name + " " + ex.Message);
        ...
        private void lblDebitAccountID_KeyDown(object sender, KeyEventArgs e)
        {
            //if (e.KeyCode == Keys.F3)
            //    btnDebitSearch_Click(null, null);
        }

        private void lblCreditAccountID_KeyDown(object sender, KeyEventArgs e)
        {
            //if (e.KeyCode == Keys.F3)
            //    btnCreditSearch_Click(null, null);
        }

        private void txtDebitGoldAccountID_KeyDown(object sender, KeyEventArgs e)
        {
            //if (e.KeyCode == Keys.F3)
            //    btnDebitGoldSearch_Click(null, null);
        }
        ...
        long SaveVariousVoucherMachin(int DocumentID)
        {
            ...
            //Crdit Gold
                if (Comon.ConvertToDecimalQty(lblInvoiceEquivalenTotal.Text) > 0)
                {
                    returned = new Acc_VariousVoucherMachinDetails();
                    returned.ID = 2;
                    returned.BranchID = Comon.cInt(cmbBranchesID.EditValue);
        ...
        public void Transaction()
        {
            ...
            strSQL = "Select * from  "+Stc_ItemsOutonBailDAL.TableName+" where Cancel=0 ";
                ...
                for (int i = 0; i <= dtSend.Rows.Count - 1; i++)
                {
                    ...
                    if (Comon.cInt(txtInvoiceID.Text) > 0)
                    {
                        ...
                        if (VoucherID == 0)
                            ...
                            Lip.ExecututeSQL("Update "+Stc_ItemsOutonBailDAL.TableName +" Set RegistrationNo =" + VoucherID + " where " + Stc_ItemsOutonBailDAL.PremaryKey + " = " + txtInvoiceID.Text + " AND BranchID=" + Comon.cInt(cmbBranchesID.EditValue));
