...
{ 
    public partial class frmGoodsOpeningOld : Edex.GeneralObjects.GeneralForms.BaseForm
    {
                ...
                if (ColName == "BarCode" || ColName == "SalePrice" || ColName == "Net" || ColName == "SizeID" || ColName == "Total" || ColName == "ItemID" || ColName == "QTY" || ColName == "CostPrice" || ColName == "CurrencyPrice")
                {
                    ...
                    if (ColName == "CurrencyPrice")
                    {
                        if (Comon.cDec(gridView2.GetRowCellValue(gridView2.FocusedRowHandle, "Net")) > 0)
                            gridView2.SetRowCellValue(gridView2.FocusedRowHandle, "CurrencyEquivalent", Comon.ConvertToDecimalPrice(Comon.cDec(e.Value) * Comon.cDec(gridView2.GetRowCellValue(gridView2.FocusedRowHandle, "SpendPrice"))).ToString());
                    ...
                    if (ColName == "QTY")
                    {
                        ...
                        if (Comon.cDec(gridView2.GetRowCellValue(gridView2.FocusedRowHandle, "CurrencyPrice")) > 0)
                            gridView2.SetRowCellValue(gridView2.FocusedRowHandle, "CurrencyEquivalent", Comon.ConvertToDecimalPrice(Comon.cDec(Total) * Comon.cDec(gridView2.GetRowCellValue(gridView2.FocusedRowHandle, "CurrencyPrice"))).ToString());
        ...
        void InitGrid()
        {
            ...
            gridView1.Columns["Bones"].Caption = "اجور";
        ...
        private void gridView1_ValidatingEditor(object sender, DevExpress.XtraEditors.Controls.BaseContainerValidateEditorEventArgs e)
        {
            ...
            else if (this.gridView1.ActiveEditor is TextEdit)
            {
                ...
                HasColumnErrors = false;
                string ColName = view.FocusedColumn.FieldName;
                if (ColName == "BarCode" || ColName == "Net" || ColName == "SizeID" || ColName == "Total" || ColName == "ItemID" || ColName == "QTY" || ColName == "CostPrice" || ColName == "Bones" || ColName == "CurrencyPrice")
                {
                    if (val == null || string.IsNullOrWhiteSpace(val.ToString()))
                    {
                        e.Valid = false;
                    ...
                    if (ColName == "Bones")
                    {
                        bool HasVat = chkForVat.Checked;
                        decimal QTY = 1;
                        decimal Bones = Comon.ConvertToDecimalPrice( Comon.ConvertToDecimalPrice(val.ToString()));
        ...
        public void ClearFields()
        {
            try
            {
               // comboTypeInvoice.SelectedIndex = 0;   
                ...
                cmbCurency.EditValue = Comon.cInt(MySession.GlobalDefaultGoodsOpeningCurencyID);
                lstDetail = new BindingList<Sales_SalesInvoiceDetails>();
                lstDetail.AllowNew = true;
                lstDetail.AllowEdit = true;
                lstDetail.AllowRemove = true;
        ...
        public void MoveRec(long PremaryKeyValue, int Direction)
        {
            try
            {
                ...
                {
                    SplashScreenManager.CloseForm(false);
                    strSQL = "SELECT TOP 1 * FROM " + Stc_GoodsOpeningDAL.TableName + " Where  BranchID= " + Comon.cInt( cmbBranchesID.EditValue) + " and Cancel =0 " ;
                    switch (Direction)
                    {
                        case xMoveFirst:
                            {
        ...
        protected override void DoNew()
        {
            ...
            {
                IsNewRecord = true;
                txtInvoiceID.Text = Stc_GoodsOpeningDAL.GetNewID(MySession.GlobalFacilityID, Comon.cInt(cmbBranchesID.EditValue), MySession.UserID).ToString();
        ...
        private bool AddItems(int Rowindex, string BarCode)
        {
            try
            {
              ...
              //  decimal SalePrice = CostPrice / 100 * MySession.GlobalPercentVat;
                decimal SalePrice;
                SalePrice = (CostPrice + Bones) * Comon.cDec(MySession.Cost);
                        ...
                        for (int i = 0; i <= gridView1.DataRowCount - 1; i++)
                        {
                            ...
                            if (dtt == null || dtt.Rows.Count <= 0)
                            {
                                ...
                                if (gridView1.GetRowCellValue(i, "DIAMOND_W").ToString() != "")
                                {
                                    ...
                                    objRecord.CaptionOpration = "بضاعة أول المدة";
       ...
       private bool AddItemsGold(int Rowindex, string BarCode)
                ...
                { 
                    ...
                    if (dt != null && dt.Rows.Count > 0)
                    {
                       ...
                       // txtVatID.Text = dt.Rows[0]["VatID"].ToString();
                        ...
                        chkPosted.Checked = Comon.cIntToBoolean(Comon.cInt(dt.Rows[0]["Posted"].ToString()));
        ...
        private void Save()
        {
           ...
           objRecord.OperationTypeName = (UserInfo.Language == iLanguage.English ? "Purchase  Invoice" : "بضاعة أول المدة");
           txtNotes.Text = (txtNotes.Text.Trim() != "" ? txtNotes.Text.Trim() : (UserInfo.Language == iLanguage.English ? "Purchase  Invoice" : "بضاعة أول المدة"));
            ...
            for (int i = 0; i <= gridView1.DataRowCount - 1; i++)
            {
                ...
                returned.CurrencyPrice = Comon.cDbl(txtCurrncyPrice.Text.ToString());
                returned.CurrencyEquivalent = Comon.cDbl(Comon.cDbl(txtCurrncyPrice.Text.ToString()) * Comon.cDbl(gridView1.GetRowCellValue(i, "SpendPrice").ToString()));
            ...
            if (listreturned.Count > 0)
            {
                ...
                string Result = Stc_GoodsOpeningDAL.InsertUsingXML(objRecord, Comon.cInt(txtEnteredByUserID.Text), IsNewRecord).ToString();
                ...
                if (Comon.cInt(Result) > 0)
                {
                    ...
                    if (VoucherID == 0)
                        ...
                        Lip.ExecututeSQL("Update " + Stc_GoodsOpeningDAL.TableName + " Set RegistrationNo =" + VoucherID + " where " + Stc_GoodsOpeningDAL.PremaryKey + " = " + txtInvoiceID.Text);
        ...
        protected override void DoDelete()
        {
            try
            {
                ...
                for (int i = 0; i <= gridView1.DataRowCount - 1; i++)
                {
                    ...
                    FlageChecKDelete = Lip.CheckTheItemIsHaveTransactionByBarCode(gridView1.GetRowCellValue(i, "BarCode").ToString(), "Stc_GoodOpeningDetails");
                    if (FlageChecKDelete)
                    {
                ...
                for (int i = 0; i <= gridView1.DataRowCount - 1; i++)
                {
                    ...
                    modelDeletDimondType.TypeOpration = 5;
                ...
                model.EditTime = Comon.cLong(Lip.GetServerTimeSerial());
                    ...
                    string Result = Stc_GoodsOpeningDAL.DeleteStc_GoodOpeningMaster(model).ToString();
                    if (Comon.cInt(Result) > 0)
                    {
                        //حذف القيد الالي
        ...
        void PrintPyCostPrice()
        {
            try
            {
                ...
                SplashScreenManager.ShowForm(this, typeof(WaitForm1), true, true, true);
                /******************** Report Body *************************/
                ReportName = "rptGoodsOpeningInvoice";
                ...
                rptForm.Parameters["ReportName"].Value = "فاتورة بضاعة أول المدة بسعر الشراء";
                ...
                for (int i = 0; i <= gridView1.DataRowCount - 1; i++)
                {
                    ...
                    row["SalePrice"] = gridView1.GetRowCellValue(i, "AdditionalValue").ToString();
            ...
            if (comboTypeInvoice.SelectedIndex == 0)
            {
                for (int i = 0; i <= gridView1.DataRowCount - 1; i++)
                {
                    ...
                    if (chkForVat.Checked == true)
                    {
                        ...
                        if (chkForVatOjor.Checked)
                            dtItem.Rows[i]["AdditionalValue"] = Comon.ConvertToDecimalPrice(gridView1.GetRowCellValue(i, "Bones").ToString()) * Comon.ConvertToDecimalPrice(MySession.GlobalPercentVat) / 100;
                        else

                            dtItem.Rows[i]["AdditionalValue"] = Comon.ConvertToDecimalPrice(gridView1.GetRowCellValue(i, "SpendPrice").ToString()) * Comon.ConvertToDecimalPrice(MySession.GlobalPercentVat) / 100;
        ...
        private void txtExcelPath_EditValueChanged(object sender, EventArgs e)
        {
        ...
        public void Transaction()
        {
            ...
            strSQL = "Select * from " + Stc_GoodsOpeningDAL.TableName + " where Cancel=0 ";
        ...
        private void cmbCurency_EditValueChanged(object sender, EventArgs e)
        {
            ...
            if (isLocalCurrncy > 1)
            {
                ...
                gridView2.Columns["CurrencyEquivalent"].Visible = true;
            }
            else
            {
                txtCurrncyPrice.Text = "1";
