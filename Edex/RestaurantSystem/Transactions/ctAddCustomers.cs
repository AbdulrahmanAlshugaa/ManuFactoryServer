...
{
    public partial class ctAddCustomers : UserControl
    {
       ...
       public  int CustomerNo = 0;
        ...
        public long GetNewAccountID()
        {
            try
            {
                ...
                string str;
                string strDigits = "";
                ParentAccountID = Lip.GetValue("SELECT AccountID FROM Acc_DeclaringMainAccounts WHERE DeclareAccountName='CustomerAccount' And BranchID=" + UserInfo.BRANCHID);
        ...
        public  void DoSave()
        {
            try
            {
                ...
                if (IsNewRecord == true)
                    DoNew();
                txtCustomerID.Text = txtAccountID.Text;
                SendKeys.Send("{ESC}");
            }
            catch (Exception ex)
            {
