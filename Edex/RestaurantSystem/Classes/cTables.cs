...
namespace Edex.RestaurantSystem.Classes
{
    ...
    {
        public readonly string TableName = "Res_Tables";
        public readonly string PremaryKey = "TableID";
        ...
        private void ReadRecord()
        {
            try
            {
                {
                    var withBlock = dt;
                    AbsorptionQTY = int.Parse(dt.Rows[0]["AbsorptionQTY"].ToString());
                    SectionID = int.Parse(dt.Rows[0]["SectionID"].ToString());
                    TableID = int.Parse(dt.Rows[0]["TableID"].ToString());
