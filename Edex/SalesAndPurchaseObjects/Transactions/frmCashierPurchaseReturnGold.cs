...
{
    public partial class frmCashierPurchaseReturnGold : Edex.GeneralObjects.GeneralForms.BaseForm
    {
        ...
        public frmCashierPurchaseReturnGold()
        {
            try
            {
        ...
        private void gridView1_ValidateRow(object sender, DevExpress.XtraGrid.Views.Base.ValidateRowEventArgs e)
        {
            try
            {
                ...
                {
                    if (col.FieldName == "BarCode" || col.FieldName == "Net" || col.FieldName == "Total" || col.FieldName == "ItemID" || col.FieldName == "QTY" || col.FieldName == "SizeID" )
                    {
        ...
        private void gridView1_ValidatingEditor(object sender, DevExpress.XtraEditors.Controls.BaseContainerValidateEditorEventArgs e)
        {
            ...
            else if (this.gridView1.ActiveEditor is TextEdit)
            {
                ...
                if (ColName == "BarCode" || ColName == "Net" || ColName == "SizeID" || ColName == "Total" || ColName == "ItemID" || ColName == "QTY" )
                {
                    if (val == null || string.IsNullOrWhiteSpace(val.ToString()))
                    {
                        e.Valid = false;
                    ...
                    if (ColName == "Net")
                    {
                        if (Comon.ConvertToDecimalPrice(val.ToString()) > 0)
                        {
                            ...
                            decimal additonalVAlue = Comon.ConvertToDecimalPrice(Net - ((Net * 100) / (100 + MySession.GlobalPercentVat)));
                            if (HasVat == true)
                                PriceUnit = Comon.ConvertToDecimalPrice((Net - additonalVAlue) / (Comon.ConvertToDecimalQty(gridView1.GetFocusedRowCellValue("QTY"))));
                            else
                                PriceUnit = Comon.ConvertToDecimalPrice((Net - additonalVAlue) / (Comon.ConvertToDecimalQty(gridView1.GetFocusedRowCellValue("QTY")) * Comon.ConvertToDecimalPrice(1.15)));
                            string BarCode = gridView1.GetRowCellValue(gridView1.FocusedRowHandle, gridView1.Columns["BarCode"]).ToString();
                            if (BarCode == "24")
                                additonalVAlue = 0;
            ...
            try
            {
                ...
                else if (e.KeyCode == Keys.Tab || e.KeyCode == Keys.Enter)
                {
                    if (view.ActiveEditor is TextEdit)
                    {
                        ...
                        string ColName = view.FocusedColumn.FieldName;
                        if (ColName == "BarCode" || ColName == "Net" || ColName == "Total" || ColName == "ItemID" || ColName == "QTY" || ColName == "SizeID" )
                        {
        ...
        private void FileItemData(DataTable dt)
        {
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                decimal AverageCost = frmItems.GetItemAverageCostPrice(Comon.cLong(dt.Rows[0]["ItemID"]), Comon.cInt(dt.Rows[0]["SizeID"]), Comon.cDbl(dt.Rows[0]["StoreID"]), Comon.cInt(dt.Rows[0]["BranchID"]), 0, 0, 0);
        ...
        private void EnabledControl(bool Value)
        {
            ...
            foreach (GridColumn col in gridView1.Columns)
            {
                if (col.FieldName == "BarCode" || col.FieldName == "Description" || col.FieldName == "Discount" || col.FieldName == "ExpiryDate" || col.FieldName == "SizeID" || col.FieldName == "ItemID" || col.FieldName == "QTY")
                {
        ...
        bool IsValidGrid()
        {
            ...
            for (int i = 0; i < length; i++)
            {
                foreach (GridColumn col in gridView1.Columns)
                {
                    if (col.FieldName == "BarCode" || col.FieldName == "Net" || col.FieldName == "Total" || col.FieldName == "SizeID" || col.FieldName == "ItemID" || col.FieldName == "QTY"  )
                    {
        ...
        public void MoveRec(long PremaryKeyValue, int Direction)
        {
            try
            {
                ...
                {  
                    SplashScreenManager.CloseForm(false);
                    strSQL = "SELECT TOP 1 * FROM " + Sales_PurchaseInvoicesReturnDAL.TableName + "  where Cancel =0 And BranchID=" + Comon.cInt(cmbBranchesID.EditValue) + "and GoldUsing=" + GoldUsing ;
                    switch (Direction)
                    {
                        case xMoveFirst:
                            {
        ...
        long SaveVariousVoucherMachin(int DocumentID)
        {
            ...
            if (Comon.cInt(cmbMethodID.EditValue.ToString()) == 1 || Comon.cInt(cmbMethodID.EditValue.ToString()) == 2)
            {
                ...
                returned.Credit = 0;
                returned.Debit = Comon.cDbl(lblInvoiceTotal.Text) + Comon.cDbl(lblAdditionaAmmount.Text);
            ...
            if (Comon.cInt(cmbMethodID.EditValue.ToString()) == 5)
            {
                ...
                returned.VoucherID = VoucherID;
                returned.Credit = 0;
                returned.Debit = (Comon.cDbl(lblInvoiceTotal.Text) + Comon.cDbl(lblAdditionaAmmount.Text)) - Comon.cDbl(txtNetAmount.Text);
                returned.DebitGold = Comon.cDbl(Comon.cDbl(returned.Credit / Comon.cDbl(lblInvoiceTotal.Text) + Comon.cDbl(lblAdditionaAmmount.Text)) * Comon.cDbl(lblInvoiceTotalGold.Text)); 
        ...
        private void Save()
        {
            ...
            objRecord.TransportDebitAccount = 0;
            ...
            //Ammount
            objRecord.TransportDebitAmount = 0;
            ...
            //image
            if (OpenFileDialog1 != null && (OpenFileDialog1.FileName != "") || picItemUnits.Image != null)
            {
                if (picItemUnits.Image != null)
                {
                    byte[] Imagebyte = imageToByteArray(picItemUnits.Image);
                ...
                else
                {
                    ...
                    picItemUnits.Image = Image.FromFile(OpenFileDialog1.FileName);
                    picItemUnits.Visible = true;
                    byte[] Imagebyte = imageToByteArray(picItemUnits.Image);
                    objRecord.InvoiceImage = Imagebyte;
                }
            }
            ...
            if (listreturned.Count > 0)
            {
                ...
                string Result = Sales_PurchaseInvoicesReturnDAL.InsertUsingXML(objRecord, IsNewRecord);
                ...
                if (Comon.cInt(Result) > 0)
                {
                    ...
                    if (SpendID == 0)
                        ...
                        Lip.ExecututeSQL("Update " + Sales_PurchaseInvoicesReturnDAL.TableName + " Set RegistrationNo =" + SpendID + " where " + Sales_PurchaseInvoicesReturnDAL.PremaryKey + " = " + txtInvoiceID.Text + " AND BranchID=" + Comon.cInt(cmbBranchesID.EditValue));
        ...
        private void PrintBill()
        {
            ...
            try
            {
                ...
                DateTime tempDate = new DateTime(year, month, day);
                rptForm.Parameters["HDate"].Value = Comon.ConvertFromEngDateToHijriDate(tempDate).Substring(0, 10);
                rptForm.Parameters["NumbToWord"].Value = Lip.ToWords(Convert.ToDecimal(lblNetBalance.Text.Trim().ToString()), Comon.cInt(MySession.GlobalDefaultSaleCurencyID));
